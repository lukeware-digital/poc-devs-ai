...
def _validate_config(config: dict) -> None:
    ...
    if missing_fields:
        raise ValueError(f"Campos obrigatórios faltando na configuração: {', '.join(missing_fields)}")
    ...
    if missing_agents:
        raise ValueError(f"Agentes faltando em 'agent_models': {', '.join(missing_agents)}")

    agents_config = config.get("agents", {})
    missing_agent_configs = [agent for agent in required_agents if agent not in agents_config]
    if missing_agent_configs:
        raise ValueError(f"Configurações faltando em 'agents': {', '.join(missing_agent_configs)}")
...
def load_configuration(config_path: str = None) -> dict:
    ...
    with open(config_file) as f:
        ...
        if config_path_str.endswith(".yaml") or config_path_str.endswith(".yml"):
            ...
            raise ValueError(
                f"Formato de configuração não suportado: {config_path_str}. Use YAML (.yaml, .yml) ou JSON (.json)."
            )
