...
def realizar_pesquisa(state: GraphState) -> GraphState:
    ...
    response = search.invoke(state.input)
...
def traduzir_para_portugues(state: GraphState) -> GraphState:
    response = llm.invoke([HumanMessage(content=f"Traduz o seguinte texto para o português: {state.output}")])
    return GraphState(input=state.input, output=response.content, tipo=state.tipo)
...
def classificar(state: GraphState) -> GraphState:
    ...
    if any(palavra in pergunta for palavra in ["soma", "quanto é", "+", "calcular"]):
        ...
        palavra in pergunta
        for palavra in ["traduzir", "traduz", "traduzir para o português", "traduzir para português"]
    ):
        tipo = "traduzir"
    else:
        tipo = "erro"
...
graph.add_node("traduzir_para_portugues", traduzir_para_portugues)
...
def route_classificar(state: GraphState) -> str:
    ...
    elif state.tipo == "traduzir":
        return "traduzir_para_portugues"
...
graph.add_edge("realizar_calculo", "traduzir_para_portugues")
graph.add_edge("responder_curiosidade", "traduzir_para_portugues")
graph.add_edge("responder_erros", "traduzir_para_portugues")
graph.add_edge("realizar_pesquisa", "traduzir_para_portugues")
graph.add_edge("traduzir_para_portugues", END)
...
if __name__ == "__main__":
    exemplos = [
        ...
        "Pesquise sobre, 'quanto tempo leva para criar um llm?'",
    ]
