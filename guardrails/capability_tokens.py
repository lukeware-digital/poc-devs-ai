...
class CapabilityToken:
    ...
    def __init__(self, token_id: str, agent_id: str, operation: str, 
                 scope: List[str], expires_at: datetime):
...
class CapabilityTokenManager:
    ...
    def generate_token(self, agent_id: str, operation: str, scope: List[str],
                      ttl: Optional[int] = None, metadata: Dict[str, Any] = None) -> CapabilityToken:
        """
        ...
        token_id = hashlib.sha256(
            f"{agent_id}{operation}{datetime.utcnow().isoformat()}{scope}".encode()
        ).hexdigest()[:16]
    ...
    def list_active_tokens(self, agent_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Lista tokens ativos
        ...
        if self.redis and len(active_tokens) < 100:  # Limite para evitar sobrecarga
            try:
                pattern = f"capability_token:*" if agent_id is None else f"capability_token:*_{agent_id}_*"
