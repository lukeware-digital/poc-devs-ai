...
from config.constants import (
    CRITICAL_OPERATIONS, PROTECTED_FILE_PATTERNS,
    ALLOWED_SYSTEM_COMMANDS, DANGEROUS_PORTS
)
...
class GuardrailSystem:
    ...
    async def check_permission(self, agent_id: str, operation: str, 
        ...
        try:
            ...
            if operation in self.critical_operations:
                ...
                if not token_id or not self.token_manager.validate_token(token_id, agent_id, operation):
                    return False, f"Token de capacidade requerido para operação crítica: {operation}"
    ...
    async def _check_file_modification_permissions(self, agent_id: str, 
        ...
        for pattern in self.protected_file_patterns:
            if fnmatch.fnmatch(os.path.basename(file_path), pattern) or fnmatch.fnmatch(file_path, pattern):
                return False, f"Modificação proibida para arquivo protegido: {file_path}"
        
        # Verifica se o conteúdo contém padrões suspeitos
        suspicious_patterns = [
            'os.system', 'subprocess.call', 'eval(', 'exec(', 
            'import os', 'import sys', 'import subprocess',
            'chmod', 'chown', 'rm -rf', 'sudo', 'apt-get',
            'curl ', 'wget ', 'nc ', 'netcat', 'telnet'
        ]
        
        for pattern in suspicious_patterns:
            if pattern in content.lower():
                return False, f"Conteúdo suspeito detectado no arquivo {file_path}: {pattern}"
        
        # Verifica tamanho do arquivo (limita arquivos muito grandes)
        if len(content) > 100000:  # 100KB
            return False, f"Tamanho de arquivo muito grande para modificação automática: {len(content)} bytes"
        
        return True, ""
    
    async def _check_file_deletion_permissions(self, agent_id: str, 
                                              context: Dict[str, Any]) -> Tuple[bool, str]:
        """
        ...
        if file_path.endswith(('.py', '.js', '.ts', '.java', '.cpp', '.h', '.go', '.rs')):
            ...
            if not any(x in file_path.lower() for x in ['temp', 'test', 'tmp', 'backup', 'old']):
                return False, f"Não é permitido deletar arquivos de código fonte principais: {file_path}"
    ...
    async def _check_system_command_permissions(self, agent_id: str, 
                                               context: Dict[str, Any]) -> Tuple[bool, str]:
        """
        ...
        # Verifica se o comando contém caracteres perigosos
        dangerous_chars = ['>', '>>', '<', '|', '&', ';', '`', '$', '(', ')', '{', '}', '[', ']', '\\', '\n', '\r']
        ...
        # WHITELIST: Permite apenas comandos explicitamente autorizados
        if command_name not in self.allowed_commands:
            return False, f"Comando '{command_name}' não está na whitelist. Permitidos: {', '.join(self.allowed_commands)}"
        
        # Verifica argumentos para acessos a diretórios sensíveis
        ...
        # Verifica tentativas de path traversal nos argumentos
        for part in command_parts[1:]:
            if '..' in part or part.startswith('/'):
                # Normaliza e verifica se está dentro do workspace
                try:
    ...
    async def _check_network_permissions(self, agent_id: str, 
                                        context: Dict[str, Any]) -> Tuple[bool, str]:
        """
        ...
        local_patterns = ['localhost', '127.0.0.1', '192.168.', '10.', '172.16.', '172.31.']
        ...
        if not url.startswith(('https://', 'wss://')):
            return False, f"Apenas requisições seguras (HTTPS/WSS) são permitidas: {url}"
        
        # Bloqueia URLs suspeitas
        suspicious_patterns = ['admin', 'login', 'password', 'credential', 'secret', 'token', 'auth']
        for pattern in suspicious_patterns:
            if pattern in url.lower():
    ...
    async def _check_git_permissions(self, agent_id: str, 
                                   context: Dict[str, Any]) -> Tuple[bool, str]:
        """
        ...
        if len(changes) > 50:
            return False, f"Muitas mudanças para push único ({len(changes)}). Limite: 50"
        
        # Verifica tamanho das mudanças
        total_size = sum(len(str(change)) for change in changes)
        if total_size > 10000:  # 10KB
            return False, f"Tamanho total das mudanças muito grande ({total_size} bytes). Limite: 10KB"
        ...
        for change in changes:
            ...
            for pattern in self.protected_file_patterns:
                if fnmatch.fnmatch(file_path, pattern):
                    return False, f"Mudança em arquivo protegido não permitida: {file_path}"
    ...
    def should_sandbox_execution(self, operation: str, context: Dict[str, Any]) -> bool:
        ...
        """
        sandbox_operations = [
            'code_execution', 'script_execution', 'plugin_execution',
            'dynamic_import', 'eval_code'
        ]
    ...
    def get_agent_permissions(self, agent_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'critical_operations_allowed': [
                op for op in self.critical_operations 
                if op not in self.agent_restrictions.get(agent_id, [])
            ],
    ...
    def get_security_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'agent_restrictions_summary': {
                agent_id: len(restrictions) 
                for agent_id, restrictions in self.agent_restrictions.items()
            }
        }