...
class MetricsCollector:
        ...
        if self.redis:
            try:
                self.redis.hset(key, mapping={
                    k: str(v) for k, v in metrics_with_timestamp.items()
                })
    ...
    def record_alert(self, alert_type: str, details: Dict[str, Any]):
        ...
        logger.log(log_level, f"🚨 ALERTA [{alert['severity']}]: {alert_type} - {json.dumps(details)}")
    ...
    def _determine_alert_severity(self, alert_type: str, details: Dict[str, Any]) -> str:
        ...
        """
        critical_alerts = ['system_crash', 'security_breach', 'data_loss', 'human_intervention_required']
        high_alerts = ['high_memory_usage', 'high_cpu_usage', 'high_disk_usage', 'service_unavailable']
    ...
    def get_agent_performance(self, agent_id: str, hours: int = 24) -> Dict[str, Any]:
        ...
        recent_errors = [
            m for m in relevant_metrics[-10:]  # Últimas 10 métricas
    ...
    def _calculate_metrics_trend(self, metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        second_success = sum(1 for m in second_half if m.get('success', m.get('success_rate', 0) > 50)) / len(second_half)
        ...
        try:
            ...
            for agent_id in self.metrics_history:
                ...
                agent_health[agent_id] = {
                    ...
                    'status': 'healthy' if perf['success_rate'] >= 80 else 'degraded' if perf['success_rate'] >= 50 else 'unhealthy'
                }
    ...
    def get_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        successful_operations = sum(
            (perf['success_rate'] / 100) * perf['data_points'] 
    ...
    def _generate_performance_recommendations(self, system_health: Dict[str, Any], 
        ...
        for agent_id, perf in agent_performance.items():
            if perf['success_rate'] < 70:
                recommendations.append(f"Revisar prompts e parâmetros do {agent_id} - taxa de sucesso baixa ({perf['success_rate']:.1f}%)")
            
            if perf['avg_response_time'] > 30:  # 30 segundos
                recommendations.append(f"Otimizar LLM calls para {agent_id} - tempo de resposta alto ({perf['avg_response_time']:.1f}s)")
