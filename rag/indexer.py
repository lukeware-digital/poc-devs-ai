...
class RAGIndexer:
    ...
    def _process_code_document(self, content: str) -> Dict[str, Any]:
        ...
        code_blocks = re.findall(r'```(?:python|javascript|typescript|java|cpp|csharp|go|rust)?\n(.*?)\n```', content, re.DOTALL)
        if not code_blocks:
    ...
    def _detect_programming_language(self, content: str) -> str:
        ...
        if any(lang in content_lower for lang in ['python', 'import ', 'def ', 'class ', 'pip install']):
            return "python"
        elif any(lang in content_lower for lang in ['javascript', 'js', 'npm install', 'function ', 'const ', 'let ']):
            return "javascript"
    ...
    def _classify_document_type(self, content: str) -> str:
        ...
        elif any(word in content_lower for word in ['deploy', 'deployment', 'ci/cd', 'pipeline', 'build', 'release']):
            return 'deployment'
        return 'other'
