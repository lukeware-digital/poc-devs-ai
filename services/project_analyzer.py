...
class ProjectAnalyzer:
    ...
    def validate_code_exists(self, path: str) -> bool:
        ...
        code_extensions = {
            ...
            '.ml', '.sh', '.bat', '.ps1', '.sql', '.html', '.css', '.vue',
            '.jsx', '.tsx', '.dart', '.lua', '.r', '.m', '.mm'
        }
    ...
    def analyze_directories(self, path: str) -> List[str]:
        project_path = Path(path)
        if not project_path.exists():
            return []
    ...
    def check_git_status(self, path: str) -> Dict[str, Any]:
        repo_path = Path(path)
