...
class HardwareProfile:
    ...
    def _determine_profile_name(self) -> str:
        ...
        profiles = [
            (
                "i9_13900k_rtx4090",
                lambda: "13900k" in cpu_model and "rtx 4090" in gpu_model and ram_gb >= 64,
            ),
            (
                "ryzen9_7950x_rtx4080",
                lambda: "7950x" in cpu_model and "rtx 4080" in gpu_model and ram_gb >= 64,
            ),
            ("m1_max", lambda: "m1 max" in cpu_model and ram_gb >= 64),
            (
                "i7_12700k_rtx3080",
                lambda: "12700k" in cpu_model and "rtx 3080" in gpu_model and ram_gb >= 32,
            ),
            (
                "ryzen7_5800x_rtx3070",
                lambda: "5800x" in cpu_model and "rtx 3070" in gpu_model and ram_gb >= 32,
            ),
            (
                "i5_12600k_rtx3060",
                lambda: "12600k" in cpu_model and "rtx 3060" in gpu_model and ram_gb >= 16,
            ),
            (
                "ryzen5_5600x_rtx3060",
                lambda: "5600x" in cpu_model and "rtx 3060" in gpu_model and ram_gb >= 16,
            ),
            ("mac_m1_pro", lambda: "m1 pro" in cpu_model and ram_gb >= 16),
            (
                "mac_m2_air",
                lambda: "m2" in cpu_model and "air" in cpu_model and ram_gb >= 8,
            ),
    ...
    def _generate_recommended_config(self) -> dict[str, any]:
        ...
        configs = {
            HardwareTier.ULTRA: {
                ...
                "llm_params": {"max_tokens": 8192, "temperature_range": [0.1, 0.9]},
                "performance": {
                    "gpu_offloading": True,
            ...
            HardwareTier.HIGH: {
                ...
                "llm_params": {"max_tokens": 4096, "temperature_range": [0.1, 0.8]},
                "performance": {
                    "gpu_offloading": True,
            ...
            HardwareTier.MEDIUM: {
                ...
                "llm_params": {"max_tokens": 2048, "temperature_range": [0.2, 0.7]},
                "performance": {
                    "gpu_offloading": True,
            ...
            HardwareTier.LOW: {
                ...
                "llm_params": {"max_tokens": 1024, "temperature_range": [0.3, 0.6]},
                "performance": {
                    "gpu_offloading": True,
                    "vram_optimization": "aggressive",
                    "model_parallelism": 1,
            ...
            HardwareTier.MINIMAL: {
                ...
                "llm_params": {"max_tokens": 512, "temperature_range": [0.4, 0.5]},
                "performance": {
        ...
        if ram_gb < 16:
            base_config["orchestrator"]["concurrent_agents"] = max(
                1, base_config["orchestrator"]["concurrent_agents"] // 2
            )
...
def _get_apple_vram() -> float:
    """Obtém VRAM de GPUs Apple (M1/M2)"""
    import re
...
def get_optimal_batch_size(hardware_profile: HardwareProfile) -> int:
    ...
    hardware_profile.system_info.get("vram_gb", 0)
    hardware_profile.system_info.get("ram_gb", 0)
...
def get_model_sharding_strategy(hardware_profile: HardwareProfile) -> dict[str, any]:
    ...
    if has_gpu and vram_gb >= 24:
        ...
        return {"strategy": "cpu_offload", "num_shards": 1, "pipeline_parallel": False}
