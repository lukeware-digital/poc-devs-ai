...
class LLMProvider(ABC):
    ...
    async def generate(self, prompt: str, temperature: float = 0.7, 
                      max_tokens: int = 2048, stop_sequences: List[str] = None) -> str:
        """
    ...
    @abstractmethod
    async def batch_generate(self, prompts: List[str], temperature: float = 0.7, 
...
class ResponseCache:
    ...
    def _get_cache_key(self, prompt: str, temperature: float, 
                       max_tokens: int, model_name: str) -> str:
        """Gera chave de cache única"""
        key_data = f"{prompt}|{temperature}|{max_tokens}|{model_name}"
        return hashlib.md5(key_data.encode()).hexdigest()
    
    def get(self, prompt: str, temperature: float, 
            max_tokens: int, model_name: str) -> Optional[str]:
    ...
    def set(self, prompt: str, temperature: float, max_tokens: int, 
...
class LLMAbstractLayer:
    ...
    async def generate_response(self, prompt: str, temperature: float = 0.7, 
                              max_tokens: int = 2048, stop_sequences: List[str] = None,
    ...
    async def batch_generate_responses(self, prompts: List[str], temperature: float = 0.7, 
                                      max_tokens: int = 2048) -> List[str]:
        """
        ...
        """
        tasks = [
            self.generate_response(prompt, temperature, max_tokens)
            for prompt in prompts
        ]
        return await asyncio.gather(*tasks)
