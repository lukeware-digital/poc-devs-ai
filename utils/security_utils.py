...
def validate_json_structure(json_data: dict[str, any], schema: dict[str, any]) -> bool:
    """
...
def _basic_json_validation(json_data: dict[str, any], schema: dict[str, any]) -> bool:
    """
    ...
    if not _validate_required_fields(json_data, schema):
        return False

    if not _validate_field_types(json_data, schema):
        return False

    return True


def _validate_required_fields(json_data: dict, schema: dict) -> bool:
...
def _validate_field_types(json_data: dict, schema: dict) -> bool:
    ...
    for field, field_schema in properties.items():
        if field in json_data:
            ...
            if expected_type:
                if not _check_field_type(field, json_data[field], expected_type):
                    return False
    return True


def _check_field_type(field: str, value: any, expected_type: str) -> bool:
    """Verifica tipo de um campo específico"""
    type_checks = {
        "string": (str, "string"),
        "number": ((int, float), "número"),
        "integer": (int, "inteiro"),
    ...
    if not isinstance(value, expected_class):
        logger.warning(f"Campo {field} esperava {type_name}, recebeu {type(value)}")
        return False
    return True
...
def hash_sensitive_data(data: str, algorithm: str = "sha256") -> str:
    ...
    try:
        ...
        raise ValueError(f"Algoritmo de hash inválido: {algorithm}") from e
...
def validate_capability_token(token: str, expected_operation: str, context: dict[str, any]) -> bool:
    """
...
def generate_capability_token(operation: str, context: dict[str, any], ttl: int = 300) -> str:
    """
